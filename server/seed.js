...
const buildProperties = (agents) => [
  {
    ...
    beds: 4, baths: 3, sqft: 2500, build: 2021, status: true,
    agent: agents[0]._id,
  },
  {
    ...
    beds: 5, baths: 5, sqft: 4500, build: 2019, status: true,
    agent: agents[1]._id,
  },
  {
    ...
    beds: 3, baths: 2, sqft: 1800, build: 2018, status: true,
    agent: agents[2]._id,
  },
  {
    ...
    beds: 4, baths: 3, sqft: 2300, build: 2020, status: true,
    agent: agents[0]._id,
  },
  {
    ...
    beds: 5, baths: 4, sqft: 3200, build: 2017, status: true,
    agent: agents[1]._id,
  },
  {
    ...
    beds: 4, baths: 4, sqft: 3800, build: 2022, status: true,
    agent: agents[2]._id,
  },
]
...
async function seed() {
  try {
    ...
    const properties = await Property.insertMany(buildProperties(agents))
    console.log(`Seeded ${properties.length} properties:`)
    properties.forEach(p => console.log(`  - ${p.name} → agent: ${agents.find(a => a._id.equals(p.agent)).name}`))
